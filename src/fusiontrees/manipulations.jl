...
function merge(f₁::FusionTree{I,0}, f₂::FusionTree{I,0}, c::I, μ=nothing) where {I}
    Nsymbol(f₁.coupled, f₂.coupled, c) > 0 ||
        throw(SectorMismatch("cannot fuse sectors $(f₁.coupled) and $(f₂.coupled) to $c"))
...
function elementary_trace(f::FusionTree{I,N}, i) where {I<:Sector,N}
    ...
    if i < N
        ...
        if i > 1
            c = f.innerlines[i - 1]
            if FusionStyle(I) isa MultiplicityFreeFusion
                coeff *= Fsymbol(a, b, dual(b), a, c, _one)
            else
                μ = f.vertices[i - 1]
                ν = f.vertices[i]
                coeff *= Fsymbol(a, b, dual(b), a, c, _one)[μ, ν, 1, 1]
            end
        end
        if f.isdual[i]
        ...
        for (f_′, coeff) in merge(fs, f_, _one, 1)
            f_′.innerlines[1] == _one || continue
            ...
            f′ = FusionTree(uncoupled′, _one, isdual′, inner′, vertices′)
            coeff *= sqrtdim(b)
            if !(f.isdual[N])
                coeff *= conj(frobeniusschur(b))
            end
...
function braid(f::FusionTree{I,N},
    ...
    if FusionStyle(I) isa UniqueFusion && BraidingStyle(I) isa SymmetricBraiding
        ...
        T = BraidingStyle(I) isa NoBraiding ? Fscalartype(I) : typeof(one(Fscalartype(I)) * one(Rscalartype(I)))
        coeff = one(T)
